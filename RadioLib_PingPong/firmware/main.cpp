...
#define VERSION_STRING "PingPong v1.08" 
...
void outputBefehlsliste() {
    ...
    printf("[I] :PING *             fordert PONG von ALLEN an\n");
    printf("[I] :PING ID            fordert PONG von ID an\n");
    ...
    printf("[I] :REBOOT             reboot remote ESP32\n");
    printf("\n");
    ...
    printf("[I] +REBOOT             reboot local ESP32\n");
...
void setup()
{
    ...
    deltatime = (mac[4] * 256 + mac[5])/50 + 100;  // Delta-Zeitraster aus MAC pseudo Random
    Serial.printf("deltatime= %lu\n", deltatime);

    TXpayload = VERSION_STRING;
    RXpayload = "";
    rssi = String(CONFIG_RADIO_FREQ,3)+" MHz";
    snr = "";
...
void loop()
{
    ...
    if (isTXLoRaStr) {
        ...
        if (state == RADIOLIB_LORA_DETECTED) { // Kanal belegt!
          ...
          else if (state == RADIOLIB_CHANNEL_FREE) { // Kanal frei!
            #ifdef RADIO_CTRL
